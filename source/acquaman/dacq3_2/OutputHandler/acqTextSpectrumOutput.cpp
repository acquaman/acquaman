...
int acqTextSpectrumOutput::pvFlags( acqKey_t key, int eventno, int pvno, const char *eflags)
{
	...
	std::string &checkSpectrum(to->pvInfo[uid]->pvproperties[std::string("isSpectrum")]);
	DEBUG(to) printf("pv[%d] isSpectrum:%s\n", uid, checkSpectrum.c_str() );

	if(checkSpectrum == "1" || checkSpectrum == "TRUE" || checkSpectrum == "true")
	{
		((pvTSPrivate *)(to->pvInfo[uid]))->isSpectrum = true;
		to->haveSpectrum = TRUE;
	}
	else
		((pvTSPrivate *)(to->pvInfo[uid]))->isSpectrum = false;
	return 0;
}
