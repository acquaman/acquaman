...
#include <stdarg.h>
static void
modifyScanControl( acqControl_t *ctlp, va_list ap)
{

	enum acqControlIndex aci;

	for(;;)
	{
		aci = va_arg(ap, enum acqControlIndex);
		switch( aci)
		{
		default:
			/* issue a warning: next time, the application might crash instead of just having a wrong value */
			return;
		case END_LIST:
			return;
		case CONTROL_PV:
			setCharPointerFromEntry(&ctlp->controlPV, va_arg(ap, char *));
			break;

		case HAVE_START:
			ctlp->haveStartVal = va_arg(ap, int);
			break;
		case HAVE_DELTA:
			ctlp->haveDeltaVal = va_arg(ap, int);
			break;
		case HAVE_FINAL:
			ctlp->haveFinalVal = va_arg(ap, int);
			break;
		case START_VAL:
			ctlp->startVal = va_arg(ap, double);
			ctlp->haveStartVal = 1;
			break;
		case DELTA_VAL:
			ctlp->deltaVal = va_arg(ap, double);
			ctlp->haveDeltaVal = 1;
			break;
		case FINAL_VAL:
			ctlp->finalVal = va_arg(ap, double);
			ctlp->haveFinalVal = 1;
			break;
		case START_MACRO:
			setCharPointerFromEntry(&ctlp->startMacro, va_arg(ap, char *));
			ctlp->haveStartVal = 1;
			break;

		case DELTA_MACRO:
			setCharPointerFromEntry(&ctlp->deltaMacro, va_arg(ap, char *));
			ctlp->haveDeltaVal = 1;
			break;
		case FINAL_MACRO:
			setCharPointerFromEntry(&ctlp->finalMacro, va_arg(ap, char *));
			ctlp->haveFinalVal = 1;
			break;
		case USE_INITIAL:
			ctlp->useInitial = va_arg(ap, int);
			break;
		case USE_FINAL:
			ctlp->useFinal = va_arg(ap, int);
			break;
		}
	}
}
...
void
acqModifyScanControlEntry( acqScan_t *scanp, int index, ...)
{

	va_list ap;
	va_start(ap, index);
	if( scanp->master->globalState == AS_OFF)
		modifyScanControl(&scanp->acqControlList[index], ap);
	va_end(ap);
}

void
acqAddScanControlEntry(acqMaster_t*master, acqScan_t *scanp, int index, ...)
{
	va_list ap;
	...
	va_start(ap, index);
	addScanControl(scanp, NULL);
	if( index >= 0)
	{
		int count;
		for( count=scanp->numControlPV-1; count >index; count--)
			scanp->acqControlList[count-1] = scanp->acqControlList[count];
		index = count;
	}
	else
		index = scanp->numControlPV-1;
