...
runAcqScan( acqScan_t *acq)
{
	...
	for(;;)
	{
		...
		runAcqAction( acq, acq->actions[ACQ_TG_DONE], NULL, 0.0);
		acq->state = AS_STANDBY;		/* ensure that a waiting task sees this task is no longer in 'run' mode */
		epicsEventSignal( acq->endEvent);
	}
}
		...
		case AA_WAIT_SCAN:
			{
				...
				if( sc)
				{
					/* ugly piece of code: in a perfect world, this code never returns a failure:
					 * the scan structure has a valid event entry, and so EpicsEventWaitWithTimeout() will
					 * never give an error code.
					 */
					for(;;)
					 {
						status = WaitForEvent( sc->endEvent, &sc->shutdown, &sc->state, sc->master);
						if( status >= 0)	/* stop flag occurs, or scan completes */
							break;
						if( hardError++ >= 100)	/* MAGIC: 100 loops is enough to convince us there is something seriously wrong */
							break;
					}
				}
			}
...
/*
 * wait for an epics event to signal. Check every .25 seconds. If the
 * event is invalid, returns right away as if a signal had occured.
 * Magic numbers: 0.25 - interval between waits
 * 8 - number of intervals to wait for
 */
static int
WaitForEvent( epicsEventId e_event, volatile int *shutdown, volatile acqState *state, acqMaster_t *master)
{
	...
	acqState startState;
	int countdown = 0;
	int success = 0;	/* count the timeouts that work ... trying to contain a bug that's causing pthread_cond_timedwait() to occasionally fail */
	int fail = 0;		/* count the timeouts that fail */
	...
	for(;;)
	{
		...
		if( status == epicsEventWaitTimeout)
		{
			 ...
			 * state. If it doesn't occur in 2 seconds, (magic numbers: 0.25 in delay, 8 in countdown)
		...
		{
			if( fail == 0 && success > 1)		/* if there have been multiple successes and one failure, retry */
			{
				master->acqStatusMessage(master, "WaitForEvent(%p,%p) - possible spurious error return\n", e_event, shutdown);
				fail++;
				continue;
			}
		...
		return 1;	/* got a success signal from epicsEventWaitWithTimeout() */
	}
}
