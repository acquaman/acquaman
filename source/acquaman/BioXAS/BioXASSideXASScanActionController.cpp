...
{
	sideConfiguration_ = configuration;

	// Set exporter option.

	AMExporterOptionXDIFormat *bioXASDefaultXAS = BioXAS::buildStandardXDIFormatExporterOption("BioXAS XAS (XDI Format)", sideConfiguration_->edge().split(" ").first(), sideConfiguration_->edge().split(" ").last(), true);
