...
bool AM0DTimestampAB::axisValues(int axisNumber, int startIndex, int endIndex, AMNumber *outputValues) const
{
	if (!isValid())
		return false;
...
void AM0DTimestampAB::setTimeUnits(const AMTime::Units &newUnits)
{
...
double AM0DTimestampAB::convertTimeValue(double mseconds, const AMTime::Units &newUnits) const
{
	return AMTime::convert(mseconds, AMTime::mSeconds, newUnits);
}
