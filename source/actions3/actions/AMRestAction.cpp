...
//	QVariantMap jdata;
//	jdata["body"] = "FIXME";
	/* Add Github Issue Label
	QVariantList jdata;
	jdata.append(QString("Complexity3"));
	QJson::Serializer jserializer;
	QByteArray jsonData = jserializer.serialize(jdata);
	*/

//	QVariantList jdata;
//	QList<int> complexities;
//	complexities << 1 << 2 << 3 << 5 << 8 << 13 << 21 << 40;
//	for(int x = 0, size = 8; x < size; x++){
//		QVariantMap complexityMap;
//		if(x == 2)
//			complexityMap.insert("selected", true);
//		else
//			complexityMap.insert("selected", false);
//		complexityMap.insert("value", complexities.at(x));
//		jdata.append(complexityMap);
//	}
	...
	repoMap.insert("value", "acquaman");
	jdata.append(repoMap);
	QVariantMap issueNumberMap;
	issueNumberMap.insert("name", "issue_number");
	issueNumberMap.insert("value", 1462);
	jdata.append(issueNumberMap);
	QVariantMap estimateValueMap;
	estimateValueMap.insert("name", "estimate_value");
	estimateValueMap.insert("value", 3);
	jdata.append(estimateValueMap);
	QVariantMap organizationMap;
	organizationMap.insert("name", "organization");
	organizationMap.insert("value", "acquaman");
	jdata.append(organizationMap);
	*/
...
//		restReply_ = networkAccessManager_->sendCustomRequest(request, "PATCH", buffer);
		qDebug() << "Going to PUT" << jsonData;
//		restReply_ = networkAccessManager_->sendCustomRequest(request, "PUT", buffer);
