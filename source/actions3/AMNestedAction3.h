#ifndef AMNESTEDACTION3_H
#define AMNESTEDACTION3_H
...
class AMNestedAction3 : public AMAction3
{
    Q_OBJECT
public:	
	/// Constructor: create an action to run the specified AMActionInfo.
    AMNestedAction3(AMActionInfo3* info, QObject *parent = 0) : AMAction3(info, parent) {}

	/// Copy constructor, to make a new runnable action just like \c other.
    AMNestedAction3(const AMNestedAction3& other) : AMAction3(other) {}
...
//	virtual AMAction* createCopy() const { return new AMNestedAction(*this); }
    ...
    AMAction3* takeSubAction(int index);
    ...
    virtual const AMAction3* subActionAt(int index) const = 0;
	/// Returns the sub-action at \c index, or 0 if \c index is out of range.
    virtual AMAction3* subActionAt(int index) = 0;
...
protected:
    ...
    virtual void insertSubActionImplementation(AMAction3 *action, int index) = 0;
    ...
    virtual AMAction3* takeSubActionImplementation(int index) = 0;

};

#endif // AMNESTEDACTION3_H
