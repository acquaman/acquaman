...
{
    bool isRepeat = contains(pvName);
    qDebug() << "PV has already been added : " << isRepeat;
...
void StripToolModel::onPVConnected(bool isConnected)
{
    if (isConnected)
    {
        //sender()->disconnect(this); // bad.
        emit pvValid(true);
        qDebug() << "Adding pv : " << addPV((AMControl*) sender()); // also bad.
