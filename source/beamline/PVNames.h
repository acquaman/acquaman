...
/*! This namespace provides definitions for all Process Variable names that we use/reference.  They are read from a configuration file on program startup,
    but defaults are provided here in case the file is missing/not created yet.  PVNames can also be stored from memory to disk with save(), but this
    should rarely be used. (Maybe by the developers for creating the file, but that's it ; )

    When adding PVs to this list, don't forget to add the actual declaration to PVNames.cpp, and add lines in save() and load() as well.
