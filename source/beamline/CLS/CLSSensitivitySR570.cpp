#include "CLSSensitivitySR570.h"

#include "beamline/AMPVControl.h"

CLSSensitivitySR570::CLSSensitivitySR570(const QString &name, const QString &baseName, QObject *parent) :
...
CLSSensitivitySR570::~CLSSensitivitySR570(){}

double CLSSensitivitySR570::value() const{
	return currentValueHelper();
}

QList<double> CLSSensitivitySR570::values() const
{
	return QList<double>() << 1 << 2 << 5 << 10 << 20 << 50 << 100 << 200 << 500;
}

QString CLSSensitivitySR570::units() const{
	return currentUnitHelper();
}

QStringList CLSSensitivitySR570::unitsList() const
{
	return QStringList() << "pA/V" << "nA/V" << "uA/V" << "mA/V";
}

double CLSSensitivitySR570::minimumValueForUnits(const QString &units) const{
	Q_UNUSED(units)

	return 1;
}

double CLSSensitivitySR570::maximumValueForUnits(const QString &units) const{
	int value;
...
void CLSSensitivitySR570::onSensitivityControlConnectedChanged(bool connected){
...
void CLSSensitivitySR570::onSensitivityControlValueChanged(double value){
	emit valueChanged();
...
bool CLSSensitivitySR570::atMinimumSensitivity() const{
	return atMinimumSensitivity_;
}

bool CLSSensitivitySR570::atMaximumSensitivity() const{
	return atMaximumSensitivity_;
}

bool CLSSensitivitySR570::increaseSensitivity(){
...
bool CLSSensitivitySR570::decreaseSensitivity(){
...
void CLSSensitivitySR570::setValueImplementation(const QString &valueArg)
{
	if(valueArg == "1 pA/V"){
...
double CLSSensitivitySR570::currentValueHelper() const{
...
QString CLSSensitivitySR570::currentUnitHelper() const{
	int currentSensitivityControlValue = (int)sensitivityControl_->value();
