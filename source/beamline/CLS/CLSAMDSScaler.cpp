...
bool CLSAMDSScaler::retrieveBufferedData(double /*seconds*/)
{
	...
	if (scalerAMDSServer && !amdsBufferName_.isEmpty() && scalerAMDSServer->bufferNames().contains(amdsBufferName_)){
		...
		return clientAppController->requestClientData(scalerAMDSServer->hostName(), scalerAMDSServer->portNumber(), amdsBufferName_, quint64(dataRequestSize), dataRequestSize, true, false);
	}
	else
		return false;
}
