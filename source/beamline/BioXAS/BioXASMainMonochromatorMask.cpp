	...
	BioXASSSRLMonochromatorMask(deviceName, parent)
{
	...
	BioXASSSRLMonochromatorMaskState *state = new BioXASSSRLMonochromatorMaskState(name()+"State", this);
	state->setState(new AMReadOnlyPVControl(QString("BL1607-5-I21:Mono:SlitsClosed"), QString("BL1607-5-I21:Mono:SlitsClosed"), this));
	setState(state);
}
