#ifndef SGMMONOCHROMATORSUPPORT_H
#define SGMMONOCHROMATORSUPPORT_H

#include "SGMGratingSupport.h"
#include "SGMUndulatorSupport.h"

namespace SGMMonochromatorSupport
{
	inline double energyFromGrating(SGMGratingSupport::GratingTranslationSelection gratingTranslationSelection,
	...
	inline double gratingAngleFromEnergy(SGMGratingSupport::GratingTranslationSelection gratingTranslationSelection,
										 double energy)
	{
		double spacing = SGMGratingSupport::spacing(gratingTranslationSelection);
		double c1 = SGMGratingSupport::c1(gratingTranslationSelection);
	...
	inline double undulatorPosition(double energy,
	...
	inline double exitSlitPosition(SGMGratingSupport::GratingTranslationSelection gratingTranslationSelection,
								   double energy)
	{
		double spacing = SGMGratingSupport::spacing(gratingTranslationSelection);
		double thetaM = SGMGratingSupport::thetaM(gratingTranslationSelection);
...
#include <QDebug>
	inline void testValue(const QString &name, double requiredEnergy, SGMGratingSupport::GratingTranslationSelection gratingTranslationSelection, double expectedAngleEncoderTarget, double expectedExitSlitPosition, double expectedUndulatorPosition, double expectedUndulatorStepValue)
	{
		// Compute all the required values
		double angleEncoderTarget = gratingAngleFromEnergy(gratingTranslationSelection, requiredEnergy);
		// Hamonic is set to the third if on the high grating with required energy >= 1400eV
		double requestedUndulatorPosition = undulatorPosition(requiredEnergy, requiredEnergy >= 1400 && gratingTranslationSelection == SGMGratingSupport::HighGrating ? SGMUndulatorSupport::ThirdHarmonic : SGMUndulatorSupport::FirstHarmonic, 0);
		double requestedExitSlitPosition = exitSlitPosition(gratingTranslationSelection, requiredEnergy);
...
#endif // SGMMONOCHROMATORSUPPORT_H
