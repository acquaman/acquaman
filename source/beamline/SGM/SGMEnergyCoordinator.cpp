...
void SGMEnergyCoordinator::onEnergyControlConnected()
{
	...
	onEnergyControlExitSlitTrackingChanged(energyControlCoordinator_->isExitSlitPositionTracking());
	...
	connect(energyControlCoordinator_, SIGNAL(exitSlitTrackingChanged(bool)),
			this, SLOT(onEnergyControlExitSlitTrackingChanged(bool)));
...
void SGMEnergyCoordinator::onEnergyControlExitSlitTrackingChanged(bool isTracking)
{
	...
	if(!newControls_->exitSlitTracking()->withinTolerance(isTrackingValue)) {

		qDebug() << QString("New Exit Slit Tracking set to %1").arg(isTracking);
		newControls_->exitSlitTracking()->move(isTrackingValue);
	}
}
...
void SGMEnergyCoordinator::onGratingTranslationOptimizationModeSetpointPVChanged(double)
{
	...
	if(newControls_->gratingTranslationOptimizationModeSetpoint()->withinTolerance(0)) {
		...
		qDebug() << "Unknown grating translation optimization value, ignoring value";
		return;
	}
...
void SGMEnergyCoordinator::onExitSlitTrackingPVChanged(double)
{
	if(newControls_->exitSlitTracking()->withinTolerance(0)) {
