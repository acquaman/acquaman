...
#ifndef REIXSRSXSMCPDATASOURCE_H
#define REIXSRSXSMCPDATASOURCE_H
...
/// This class exposes an array process variable as an AMDataSource, for use with REIXSRSXSMCPDetector. The first (0) axis is the energy axis, and the second (1) axis is the vertical detector axis.
/*! \todo Should there be an abstract class above this one, that doesn't tie this to an EPICS PV implementation? */
class REIXSRSXSMCPDataSource : public QObject, public AMDataSource {
	Q_OBJECT
public:
	...
	virtual ~REIXSRSXSMCPDataSource();
	REIXSRSXSMCPDataSource(const QString& name, AMReadOnlyPVControl* imagePV, int sizeX = 128, int sizeY = 128, QObject* parent = 0);
...
#endif // REIXSRSXSMCPDATASOURCE_H
