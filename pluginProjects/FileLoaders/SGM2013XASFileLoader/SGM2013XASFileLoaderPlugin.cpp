...
bool SGM2013XASFileLoaderPlugin::load(AMScan *scan, const QString &userDataFolder){
	...
	*/
	...
	qDebug() << "Trying to read a file as sgm2013XAS";
	...
	while(!fs.atEnd()){
		...
		if(line == "Start Info")
			...
			if(dataIndex == -1){
				qDebug() << "Going to insert at " << insertionIndex << dataValue;
				scan->rawData()->beginInsertRows(1, -1);
				scan->rawData()->setAxisValue(0, insertionIndex, dataValue); // insert eV
			}
			else if(dataIndex == -27){
				qDebug() << "Found the end of the row";
				scan->rawData()->endInsertRows();
				insertionIndex++;
			}
			else{
				qDebug() << "0D datasource " << insertionIndex << ", " << dataIndex << " as " << dataValue;
	...
	for(int x = 0; x < scan->rawData()->measurementCount(); x++){
		oneMeasurementInfo = scan->rawData()->measurementAt(x);
		qDebug() << x << " " << oneMeasurementInfo.rank() << " " << oneMeasurementInfo.name << " " << oneMeasurementInfo.description << " " << oneMeasurementInfo.units;
		if(oneMeasurementInfo.rank() == 1)
			qDebug() << " " << oneMeasurementInfo.axes.at(0).name << " " << oneMeasurementInfo.axes.at(0).size << " " << oneMeasurementInfo.axes.at(0).description << " " << oneMeasurementInfo.axes.at(0).units;
	}
	...
	while(!sfs.atEnd()){
		...
		if(measurementIndex == -1){
			...
			qDebug() << scan->rawData()->measurementAt(measurementIndex).name << " size is " << measurementSize;
			dataValues.clear();
	...
	while(!fs.atEnd() && !line.startsWith("#(1) "))
		line = fs.readLine();
	if(fs.atEnd()) {
		AMErrorMon::report(AMErrorReport(0, AMErrorReport::Serious, -2, "SGM2013XASFileLoader parse error while loading scan data from file. Missing #(1) event line."));
	...
	for(int i=0; i<colNames1.count(); i++){
		if(!pv2columnName(colNames1[i])){
			AMErrorMon::report(AMErrorReport(0, AMErrorReport::Serious, -2, QString("SGM2013XASFileLoader parse error while loading scan data from file. Unknown PV in header at %1 .").arg(i)));
	...
	if(eVIndex < 0) {
		AMErrorMon::report(AMErrorReport(0, AMErrorReport::Serious, -3, "SGM2013XASFileLoader parse error while loading scan data from file. I couldn't find the energy (eV) column."));
		return false;	// bad format; no primary column

	}
	*/

	// what's this? scan->clearRawDataPointsAndMeasurements();
		...
		for(int x = 0; x < scanSize; x++){
			...
			for(int y = 0; y < allSpecValues.count(); y++){
				...
				if(allSpecCounters.at(y) != allSpecSizes.at(y)) {
					AMErrorMon::report(AMErrorReport(0, AMErrorReport::Alert, -1, QString("SGM2013XASFileLoader found corrupted data in the SDD spectra file '%1' on row %2. There should be %3 elements in the spectra, but we only found %4").arg(spectraFile).arg(x).arg(allSpecSizes.at(y)).arg(allSpecCounters.at(y))));
				}
				allSpecCounters[y] = 0;
			}
		}
